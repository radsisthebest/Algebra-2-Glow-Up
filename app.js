...
function showView(name){$$(".view").forEach(v=>v.classList.add("hidden"));$(`#${name}View`).classList.remove("hidden");$$("[data-view]").forEach(b=>b.classList.toggle("active",b.dataset.view===name));if(name==="dashboard")renderDashboard();if(name==="home")updateResumeButton();scrollTo(0,0)}
function updateResumeButton(){let b=$("#resumeExam");if(b)b.classList.toggle("hidden",!data.draft)}
function resumeExam(){if(!data.draft){toast("No saved test yet.");return}exam=data.draft;applyProfile();showView("exam");$("#examStudent").textContent=data.profile.voice==="bestie"?`${data.profile.name||"Bestie"}, let's finish this.`:`${data.profile.name||"Student"}'s saved exam`;renderQuestion();startTimer()}
...
function chooseTopicQuestions(topic,count){return Array.from({length:count},(_,i)=>{let q=generatedTopicQuestion(topic,i);if(topic==="Exponential Functions"&&/log/i.test(`${q.prompt} ${q.lesson} ${q.explanation}`)){q=generatedTopicQuestion(topic,i+100)}return q})}
...
$$("[data-view]").forEach(b=>b.onclick=()=>showView(b.dataset.view));$("#setupForm").onsubmit=startExam;$("#resumeExam").onclick=resumeExam;$("#themeSelect").onchange=e=>{document.documentElement.dataset.theme=e.target.value;data.profile.theme=e.target.value;save()};["examMode","topicSelect"].forEach(id=>$(`#${id}`).onchange=updateSetupMode);["topicQuestionCount","topicTestMinutes"].forEach(id=>$(`#${id}`).oninput=updateSetupMode);$("#nextButton").onclick=next;$("#prevButton").onclick=()=>{if(exam.index){exam.index--;renderQuestion()}};$("#teachButton").onclick=teach;$("#formulaTop").onclick=openFormula;$("#formulaExam").onclick=openFormula;$("#closeFormula").onclick=()=>$("#formulaModal").close();$("#exitExam").onclick=()=>{clearInterval(timerId);data.draft=exam;save();showView("home");toast("Exam saved. You can resume it from Home.")};$("#toggleReview").onclick=()=>{let x=$("#reviewList");x.classList.toggle("hidden");$("#toggleReview").textContent=x.classList.contains("hidden")?"Show review":"Hide review"};$("#clearData").onclick=()=>{if(confirm("Clear your profile and all exam results from this browser?")){localStorage.removeItem(KEY);data=load();applyProfile();renderDashboard();updateResumeButton()}};applyProfile();updateResumeButton();renderDashboard();if("serviceWorker"in navigator&&location.protocol.startsWith("http")){let refreshed=false;navigator.serviceWorker.addEventListener("controllerchange",()=>{if(refreshed)return;refreshed=true;location.reload()});navigator.serviceWorker.register("./sw.js?v=14").then(reg=>reg.update()).catch(()=>{})}
